...
/**
 ...
 注意：
        ...
        2.使用tableView.contentInsetAdjustmentBehavior时要注意
        3.再使用MJRefresh刷新停止时导航条会出现跳动bug,这个bug是MJRefresh中的，因为UITableViewController自带的UIRefreshControl刷新控件就不会。
          如果出现这种问题请使用其它方案，比如：TKSDKListViewController，TKSDKViewController+UITableView等方式实现。
